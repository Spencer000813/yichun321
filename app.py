...
class ScheduleManager:
    ...
    def add_schedule(self, date_str, time_str, content, user_id, reminder=None):
        ...
        try:
            ...
            row = [date_str, time_str or '', content, reminder or '', created_time, user_id]
    ...
    def get_two_weeks_later_schedules(self):
        ...
        try:
            ...
            for record in all_records:
                if not record.get('日期') or not record.get('行程內容') or not record.get('LINE用戶ID'):
                    continue
...
def handle_message(event):
    ...
    try:
        ...
        if text.startswith("倒數") and "分鐘" in text:
            try:
                ...
                if 0 < minute <= 60:
                    ...
                    # 計算剩餘時間後自動推送訊息
                    target_id = event.source.group_id if hasattr(event.source, 'group_id') and event.source.group_id else event.source.user_id
        ...
        try:
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text="系統暫時無法處理您的請求，請稍後再試")
            )
        except:
            pass
...
def friday_reminder():
    ...
    try:
        ...
        for user_id, schedules in schedules_by_user.items():
            if schedules:
                message = "🔔 兩週後行程提醒\n\n"
                for schedule in sorted(schedules, key=lambda x: (x['日期'], x.get('時間', ''))):
                    date = schedule.get('日期', '')
                    time = schedule.get('時間', '') or '全天'
                    content = schedule.get('行程內容', '')
                    if time != '全天':
                ...
                try:
                    ...
                    print(f"成功推播提醒給用戶: {user_id}")
                except Exception as e:
                    print(f"推播失敗 {user_id}: {e}")
        
        print(f"週五提醒執行完成，共推播給 {len(schedules_by_user)} 位用戶")
    except Exception as e:
        print(f"週五提醒執行失敗: {e}")
